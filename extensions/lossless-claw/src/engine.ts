...
import { DEFAULT_CRITICAL_BUDGET_PRESSURE_RATIO, describeLcmConfigSource } from "./db/config.js";
...
function serializeRawPayloadContent(
  message: AgentMessage,
...
function buildMessageParts(params: {
  ...
  const originalByteSize =
    typeof topLevel.originalByteSize === "number" ? topLevel.originalByteSize : undefined;
...
function firstRuntimeTokenCount(
 ...
 */
function sumPromptTokensFromUsageRecord(
  record: Record<string, unknown> | null,
): number | undefined {
  if (!record) {
    return undefined;
...
function extractRuntimePromptTokenCount(
  runtimeContext?: Record<string, unknown>,
): number | undefined {
  ...
  const usageSum = sumPromptTokensFromUsageRecord(
    asRecord(ctx["usage"]) ?? asRecord(ctx["lastCallUsage"]),
  );
  ...
  const promptCacheUsageSum = sumPromptTokensFromUsageRecord(
    asRecord(promptCache?.["lastCallUsage"]),
  );
...
export class LcmContextEngine implements ContextEngine {
  ...
  private latestPromptCacheTouchSignalAt(
    ...
    const candidates = [telemetry?.lastCacheTouchAt, telemetry?.lastObservedCacheHitAt].filter(
  ...
  private hasFreshPromptCacheBreak(
    ...
    return Boolean(
      telemetry?.lastObservedCacheBreakAt &&
      (!lastCacheTouchSignalAt || telemetry.lastObservedCacheBreakAt >= lastCacheTouchSignalAt),
    );
  }
  ...
  private shouldDelayPromptMutatingDeferredCompaction(
    ...
    return (
      this.isPromptCacheMutationSensitiveFamily(telemetry) &&
  ...
  private resolveDeferredLeafCompactionExecutionDecision(params: {
    ...
    if (
      !this.shouldForceDeferredPromptCacheLeafCompaction(
        params.telemetry,
        params.leafDecision,
        params.currentTokenCount,
        params.tokenBudget,
  ...
  private readPromptCacheSnapshot(
    ...
    const model =
      safeString(runtimeContext?.model)?.trim() ?? safeString(runtimeContext?.modelId)?.trim();
    ...
    return {
      ...
      ...(promptTokenCount !== undefined ? { lastObservedPromptTokenCount: promptTokenCount } : {}),
      cacheState,
      ...(retention ? { retention } : {}),
      sawExplicitBreak,
  ...
  private async updateCompactionTelemetry(params: {
    ...
    await this.compactionTelemetryStore.upsertConversationCompactionTelemetry({
      ...
      lastObservedCacheRead:
        snapshot?.lastObservedCacheRead ?? existing?.lastObservedCacheRead ?? null,
      ...
      lastObservedCacheHitAt:
        snapshot?.cacheState === "hot" ? now : (existing?.lastObservedCacheHitAt ?? null),
  ...
  private async evaluateIncrementalCompaction(params: {
    ...
    if (
      ...
      leafTrigger.rawTokensOutsideTail <
        Math.floor(leafTrigger.threshold * this.config.cacheAwareCompaction.hotCachePressureFactor)
    ) {
      return this.logIncrementalCompactionDecision({
        conversationId: params.conversationId,
        cacheState,
      ...
      const result =
        maintenance.reason?.trim() === "threshold"
          ...
          : await (async (): Promise<CompactResult> => {
              ...
              const executionLeafDecision = this.resolveDeferredLeafCompactionExecutionDecision({
                telemetry,
                leafDecision,
                currentTokenCount: resolvedCurrentTokenCount,
                tokenBudget: resolvedTokenBudget,
  ...
  private async maybeConsumeDeferredCompactionDebtForAssemble(params: {
    ...
    await this.withSessionQueue(
      ...
      async () => {
        ...
        const promptOverflowEmergency = (normalizedCurrentTokenCount ?? 0) > cappedTokenBudget;
        if (
  ...
  private async executeCompactionCore(params: CompactionExecutionParams): Promise<CompactResult> {
    ...
    const effectiveCurrentTokens =
      observedTokens !== undefined ? observedTokens : forceCompaction ? tokenBudget : undefined;
  ...
  private static isExternalizedReferenceContent(value: string): boolean {
    ...
    return (
      ...
      /\[(?:User|Tool|Assistant|Image) image: [^\]]*LCM file: file_[a-f0-9]{16}\]/.test(trimmed)
    );
  }
  ...
  private setStableOrphanStrippingOrdinal(conversationId: number, ordinal: number): void {
    ...
    while (
      this.stableOrphanStrippingOrdinalsByConversation.size > MAX_STABLE_ORPHAN_STRIPPING_BOUNDARIES
    ) {
      const oldestConversationId = this.stableOrphanStrippingOrdinalsByConversation
        .keys()
        .next().value;
      if (typeof oldestConversationId !== "number") {
        break;
  ...
  async bootstrap(params: {
    ...
    if (this.config.pruneHeartbeatOk && result.bootstrapped === false) {
      try {
        ...
        this.deps.log.warn(`[lcm] bootstrap: heartbeat pruning failed: ${describeLogError(err)}`);
      }
    }
  ...
  async assemble(params: {
    ...
    try {
      ...
      const stableOrphanStrippingOrdinal =
        cacheAwareState === "hot"
  ...
  private async executeLeafCompactionCore(params: {
    ...
    return {
      ok: !authFailure,
      compacted: rounds > 0,
      reason: authFailure ? "provider auth failure" : rounds > 0 ? "compacted" : "below threshold",
      result: {
        tokensBefore,
  ...
  > {
    ...
    return {
      ...
      candidate: {
        ...
        conversationId: conversation.conversationId,
        sizeBytes,
        currentMessageCount: await this.conversationStore.getMessageCount(
          conversation.conversationId,
        ),
      },
    };
    ...
    const queueKeys = Array.from(
      new Set(
        candidates.map((candidate) =>
          this.resolveSessionQueueKey(candidate.sessionId, candidate.sessionKey),
        ),
      ),
    ).sort();
          ...
          async () => {
            ...
            for (const candidate of params.candidates) {
              ...
              if (rotateResult.checkpointSize >= params.thresholdBytes) {
                this.oversizedAutoRotateCheckpointByQueueKey.set(
                  queueKey,
                  rotateResult.checkpointSize,
                );
              } else {
                this.oversizedAutoRotateCheckpointByQueueKey.delete(queueKey);
              }
  ...
  private async rewriteTranscriptForRotate(params: {
    ...
    const serialized =
      [JSON.stringify(header), ...linearizedEntries.map((entry) => JSON.stringify(entry))].join(
        "\n",
      ) + "\n";
  ...
  async rotateSessionStorage(params: {
    ...
    this.ensureMigrated();
    return this.withSessionQueue(this.resolveSessionQueueKey(sessionId, sessionKey), async () =>
