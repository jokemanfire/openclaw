  ...
  return {
      ...
      try {
        ...
        const configError =
          ...
          err.message.startsWith(PROVIDER_API_RESOLUTION_ERROR_PREFIX)
            ...
            : undefined;
        const providerError =
          !authError && !configError ? detectProviderBridgeError(err) : undefined;
        return {
          content: [],
    ...
    resolveSessionTranscriptFile: async ({ sessionId, sessionKey }) => {
      ...
      try {
        ...
        const transcriptPath = sessionApi.resolveSessionFilePath(normalizedSessionId, entry, {
          agentId,
          storePath,
        });
    ...
    listStartupSessionFileCandidates: async () => {
      ...
      for (const agentId of listConfiguredAgentIds(cfg)) {
        ...
        for (const [rawSessionKey, rawEntry] of Object.entries(store)) {
          ...
          try {
            sessionFile = sessionApi
              .resolveSessionFilePath(sessionId, rawEntry, {
                agentId,
                storePath,
              })
              .trim();
