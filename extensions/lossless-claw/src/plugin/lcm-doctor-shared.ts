 ...
 */
export function loadDoctorTargets(db: DatabaseSync, conversationId?: number): DoctorTargetRecord[] {
  const statement =
  ...
  const rows = (
    conversationId === undefined
      ...
      : statement.all(conversationId, FALLBACK_SUMMARY_MARKER, TRUNCATED_SUMMARY_PREFIX)
  ) as DoctorTargetRow[];
