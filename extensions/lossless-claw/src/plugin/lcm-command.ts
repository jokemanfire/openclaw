...
function parseDoctorCleanerApplyArgs(
  tokens: string[],
): { ok: true; filterId?: DoctorCleanerId; vacuum: boolean } | { ok: false; error: string } {
...
function parseLcmCommand(rawArgs: string | undefined): ParsedLcmCommand {
  ...
  switch (head.toLowerCase()) {
    ...
    case "doctor":
      ...
      return {
        kind: "help",
        error: `\`${VISIBLE_COMMAND} doctor\` accepts no arguments, \`clean\` for global high-confidence junk diagnostics, \`clean apply [filter-id] [vacuum]\` for cleanup, or \`apply\` for the scoped summary repair path.`,
      };
    case "help":
      return { kind: "help" };
    default:
...
async function resolveCurrentConversation(params: {
  ...
  return {
    kind: "unavailable",
    reason:
      "OpenClaw did not expose an active session key or session id here, so only GLOBAL stats are available.",
  };
}
...
function buildHelpText(error?: string): string {
  const lines = [
    ...
    buildSection("📘 Commands", [
      ...
      buildStatLine(
        formatCommand(`${VISIBLE_COMMAND} doctor`),
        "Scan for broken or truncated summaries.",
      ),
      buildStatLine(
      ...
      buildStatLine(
        formatCommand(`${VISIBLE_COMMAND} doctor apply`),
        "Repair broken summaries in the current conversation.",
      ),
    ]),
    "",
    buildSection("🧭 Notes", [
      buildStatLine(
        "subcommands",
        `Discover them with ${formatCommand(`${VISIBLE_COMMAND} help`)}.`,
      ),
      buildStatLine("alias", `${formatCommand(HIDDEN_ALIAS)} is accepted as a shorter alias.`),
      buildStatLine(
        "current conversation",
        "Uses the active LCM session when the host exposes session identity.",
      ),
      buildStatLine(
        "`/new`",
        "Prunes context for the current LCM conversation. It does not split storage.",
      ),
      buildStatLine(
        "`/reset`",
        "Resets OpenClaw session flow. Use rotate when you only want transcript compaction.",
      ),
    ]),
  ];
  return lines.join("\n");
...
function buildDoctorCleanerExampleLine(params: {
  ...
  const sessionKey = params.sessionKey
    ? formatCommand(truncateMiddle(params.sessionKey, 44))
    : "missing";
  const preview = params.firstMessagePreview
    ? ` · first: ${JSON.stringify(params.firstMessagePreview)}`
    : "";
...
async function buildStatusText(params: {
  ...
  const lines = [
    ...
    buildSection("🧩 Plugin", [
      ...
      buildStatLine(
        "selected",
        `${formatBoolean(selected)}${slot ? ` (slot=${slot})` : " (slot=unset)"}`,
      ),
  ...
  if (current.kind === "resolved") {
    ...
    lines.push(
      buildSection("📍 Current conversation", [
        ...
        buildStatLine(
          "session key",
          current.stats.sessionKey
            ? formatCommand(truncateMiddle(current.stats.sessionKey, 44))
            : "missing",
        ),
        ...
        buildStatLine(
          ...
          formatNumber(current.stats.summarizedSourceTokens),
        ),
        buildStatLine("tokens in context", formatNumber(current.stats.contextTokenCount)),
        buildStatLine(
          "compression ratio",
          formatCompressionRatio(
            current.stats.contextTokenCount,
            current.stats.compressedTokenCount,
          ),
        ),
        buildStatLine(
          "doctor",
    ...
    lines.push(
      ...
      buildSection("🛠️ Maintenance", [
        buildStatLine(
          "state",
          maintenance?.pending ? "pending" : maintenance?.running ? "running" : "idle",
        ),
        ...
        buildStatLine(
          ...
          maintenance?.currentTokenCount != null
            ? formatNumber(maintenance.currentTokenCount)
            : "unknown",
        ),
        buildStatLine("last api call", formatMaintenanceTime(telemetry?.lastApiCallAt ?? null)),
        buildStatLine(
          "last cache touch",
          formatMaintenanceTime(telemetry?.lastCacheTouchAt ?? null),
        ),
        buildStatLine("cache retention", telemetry?.retention ?? "unknown"),
        buildStatLine("cache state", telemetry?.cacheState ?? "unknown"),
        buildStatLine(
          "provider/model",
          [telemetry?.provider, telemetry?.model].filter(Boolean).join(" / ") || "unknown",
        ),
      ]),
    );
  } else {
...
async function buildDoctorCleanersText(params: { db: DatabaseSync }): Promise<string> {
...
async function buildBackupText(params: { db: DatabaseSync; config: LcmConfig }): Promise<string> {
  const lines = [...buildHeaderLines(), "", "💾 Lossless Claw Backup", ""];
...
async function buildRotateText(params: {
  ...
  const lines = [...buildHeaderLines(), "", "🪓 Lossless Claw Rotate", ""];
...
async function buildDoctorCleanersApplyText(params: {
  ...
  lines.splice(
    ...
    buildSection("📊 Current matches", [
      buildStatLine(
        "matched conversations before apply",
        formatNumber(before.totalDistinctConversations),
      ),
