...
export type DoctorCleanerId = "archived_subagents" | "cron_sessions" | "null_subagent_context";
...
export function getDoctorCleanerFilters(): Array<
  Pick<DoctorCleanerFilterStat, "id" | "label" | "description">
> {
...
function stageCleanerConversationIds(db: DatabaseSync, definitions: CleanerDefinition[]): void {
  ...
  const matchedConversationsSql = buildMatchedConversationsSql({
    ...
    messageStatsTableName: needsFirstMessage ? "temp.doctor_cleaner_first_messages" : undefined,
  });
  db.exec(`
