...
export async function applyScopedDoctorRepair(params: {
  ...
  if (!summarize) {
    return {
      kind: "unavailable",
      reason:
        "Lossless Claw could not resolve a summarizer for native doctor apply through the normal model/auth chain.",
    };
  }
  ...
  for (const target of ordered) {
    try {
      ...
      const rewritten = (
        await summarize(sourceText, false, {
          previousSummary,
...
function previousViaContextItems(params: {
  ...
  const previousRow = params.db
    ...
    .get(params.target.conversationId, params.target.depth, targetRow.ordinal) as
    | { summary_id: string }
    | undefined;
...
function resolveSummaryTimeRange(params: {
  ...
  const earliestAt =
    parseSqliteTimestamp(params.earliestAt) ?? parseSqliteTimestamp(params.createdAt);
...
function updateSummaryFts(db: DatabaseSync, summaryId: string, content: string): void {
  try {
    ...
    if (Number(update.changes ?? 0) === 0) {
      db.prepare(`INSERT INTO summaries_fts(summary_id, content) VALUES (?, ?)`).run(
        summaryId,
        content,
      );
    }
