...
function mergeMaintenanceRecord(
  ...
  return {
    ...
    requestedAt:
      patch.requestedAt !== undefined ? patch.requestedAt : (existing?.requestedAt ?? null),
    ...
    tokenBudget:
      patch.tokenBudget !== undefined ? patch.tokenBudget : (existing?.tokenBudget ?? null),
    currentTokenCount:
      patch.currentTokenCount !== undefined
        ? patch.currentTokenCount
        : (existing?.currentTokenCount ?? null),
    updatedAt: new Date(),
  };
}
