...
export class LcmProviderAuthError extends Error {
  ...
  constructor(params: { provider: string; model: string; failure: ProviderAuthFailure }) {
...
class LcmProviderResponseError extends Error {
  ...
  constructor(params: { provider: string; model: string; failure: ProviderResponseFailure }) {
...
export function extractProviderAuthFailure(
  ...
  if (opts?.requireStructuralSignal) {
    ...
    statusCode !== 401 &&
    !hasScopeSignal &&
    !AUTH_ERROR_TEXT_PATTERN.test(normalizedMessage)
  ) {
    return undefined;
  }
...
function resolveTargetTokens(params: {
  ...
  if (mode === "aggressive") {
    const aggressiveCap = Math.max(
      96,
      Math.min(leafTargetTokens, Math.floor(leafTargetTokens * 0.55)),
    );
  ...
  const resolutionCandidates: SummaryResolutionCandidate[] = [
    {
      ...
      providerHint: process.env.LCM_SUMMARY_PROVIDER?.trim() || providerHint || undefined,
...
export async function createLcmSummarizeFromLegacyParams(params: {
  ...
  const leafTargetTokens =
    Number.isFinite(params.deps.config.leafTargetTokens) && params.deps.config.leafTargetTokens > 0
  ...
  const fn: LcmSummarizeFn = async (
    ...
    for (let index = 0; index < resolvedCandidates.length; index += 1) {
      ...
      const nextCandidate =
        index < resolvedCandidates.length - 1 ? resolvedCandidates[index + 1]! : undefined;
      ...
      const retryWithoutModelAuth = async (
        ...
        const runtimeManagedAuth =
          params.deps.isRuntimeManagedAuthProvider?.(provider, providerApi) === true;
        ...
        try {
          ...
          if (
            directErr instanceof LcmProviderAuthError ||
            directErr instanceof LcmProviderResponseError
          ) {
            throw directErr;
          }
