...
function capSummaryText(content: string, originalTokens: number, maxTokens: number): string {
  const suffixes = [
    ...
    if (incrementalMaxDepth > 0 && input.allowCondensedPasses !== false) {
      for (let targetDepth = 0; targetDepth < incrementalMaxDepth; targetDepth++) {
        ...
        const passTokensAfter =
          passTokensBefore - condenseResult.removedTokens + condenseResult.addedTokens;
        await this.persistCompactionEvents({
          conversationId,
