...
function ensureCompactionTelemetryColumns(db: DatabaseSync): void {
  ...
  if (!hasLastObservedPromptTokenCount) {
    db.exec(
      `ALTER TABLE conversation_compaction_telemetry ADD COLUMN last_observed_prompt_token_count INTEGER`,
    );
  }
}
...
function runMigrationStep(name: string, log: MigrationLogger | undefined, step: () => void): void {
    ...
    if (fts5Available) {
      ...
      runMigrationStep("ensureSummariesFts", log, () => {
        ensureStandaloneFtsTable(db, {
          ...
          staleSchemaPatterns: ["content_rowid='summary_id'", 'content_rowid="summary_id"'],
        });
      });
