 ...
 * usage.
 */
export const DEFAULT_CRITICAL_BUDGET_PRESSURE_RATIO = 0.7;
...
function toFallbackProviderArray(
  value: unknown,
): Array<{ provider: string; model: string }> | undefined {
...
function resolvePatternArray(params: { envValue: string | undefined; pluginValue: unknown }): {
  patterns: string[];
...
export function resolveLcmConfigWithDiagnostics(
  ...
  const proactiveThresholdCompactionMode =
    toProactiveThresholdCompactionMode(env.LCM_PROACTIVE_THRESHOLD_COMPACTION_MODE) ??
  ...
  const resolvedLeafChunkTokens =
    parseFiniteInt(env.LCM_LEAF_CHUNK_TOKENS) ?? toNumber(pc.leafChunkTokens) ?? 20000;
  ...
  return {
    config: {
      ...
      contextThreshold:
        parseFiniteNumber(env.LCM_CONTEXT_THRESHOLD) ?? toNumber(pc.contextThreshold) ?? 0.75,
      freshTailCount: parseFiniteInt(env.LCM_FRESH_TAIL_COUNT) ?? toNumber(pc.freshTailCount) ?? 64,
      freshTailMaxTokens:
        parseFiniteInt(env.LCM_FRESH_TAIL_MAX_TOKENS) ??
        toNumber(pc.freshTailMaxTokens) ??
        undefined,
      promptAwareEviction:
      ...
      newSessionRetainDepth:
        parseFiniteInt(env.LCM_NEW_SESSION_RETAIN_DEPTH) ?? toNumber(pc.newSessionRetainDepth) ?? 2,
      leafMinFanout: parseFiniteInt(env.LCM_LEAF_MIN_FANOUT) ?? toNumber(pc.leafMinFanout) ?? 8,
      condensedMinFanout:
        parseFiniteInt(env.LCM_CONDENSED_MIN_FANOUT) ?? toNumber(pc.condensedMinFanout) ?? 4,
      condensedMinFanoutHard:
        parseFiniteInt(env.LCM_CONDENSED_MIN_FANOUT_HARD) ??
        toNumber(pc.condensedMinFanoutHard) ??
        2,
      incrementalMaxDepth:
        parseFiniteInt(env.LCM_INCREMENTAL_MAX_DEPTH) ?? toNumber(pc.incrementalMaxDepth) ?? 1,
      leafChunkTokens: resolvedLeafChunkTokens,
      bootstrapMaxTokens: resolvedBootstrapMaxTokens,
      leafTargetTokens:
        parseFiniteInt(env.LCM_LEAF_TARGET_TOKENS) ?? toNumber(pc.leafTargetTokens) ?? 2400,
      condensedTargetTokens:
        parseFiniteInt(env.LCM_CONDENSED_TARGET_TOKENS) ??
        toNumber(pc.condensedTargetTokens) ??
        2000,
      maxExpandTokens:
        parseFiniteInt(env.LCM_MAX_EXPAND_TOKENS) ?? toNumber(pc.maxExpandTokens) ?? 4000,
      ...
      summaryTimeoutMs:
        parseFiniteInt(env.LCM_SUMMARY_TIMEOUT_MS) ?? toNumber(pc.summaryTimeoutMs) ?? 60000,
      ...
      circuitBreakerThreshold:
        ...
        toNumber(pc.circuitBreakerThreshold) ??
        5,
      circuitBreakerCooldownMs:
        parseFiniteInt(env.LCM_CIRCUIT_BREAKER_COOLDOWN_MS) ??
        toNumber(pc.circuitBreakerCooldownMs) ??
        1_800_000,
      fallbackProviders:
