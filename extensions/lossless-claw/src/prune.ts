...
function loadPruneCandidates(
  ...
  const sql =
    limit == null ? SELECT_PRUNE_CANDIDATES_SQL : `${SELECT_PRUNE_CANDIDATES_SQL}\n LIMIT ?`;
  const rows = (
    limit == null ? db.prepare(sql).all(cutoffDate) : db.prepare(sql).all(cutoffDate, limit)
 ...
 */
function stageCandidateConversationIds(db: DatabaseSync, candidates: PruneCandidate[]): void {
 ...
 */
export function pruneConversations(db: DatabaseSync, options: PruneOptions): PruneResult {
