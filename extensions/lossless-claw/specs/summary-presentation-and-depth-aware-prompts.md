...
function backfillTimeRanges(db: DatabaseSync): void {
  ...
  const sample = db
    .prepare(`SELECT earliest_at FROM summaries WHERE earliest_at IS NOT NULL LIMIT 1`)
    .get();
  if (sample) return;
  ...
  const maxDepth =
    (
      db.prepare(`SELECT MAX(depth) as d FROM summaries WHERE kind = 'condensed'`).get() as {
        d: number | null;
      }
    )?.d ?? 0;
...
async function formatSummaryContent(
  ...
  const attrs = [
    ...
    `depth="${summary.depth}"`,
    summary.descendantCount > 0 ? `descendants="${summary.descendantCount}"` : null,
  ]
    .filter(Boolean)
    .join(" ");
