...
describe("FTS fallback", () => {
  ...
  it("bypasses FTS MATCH for CJK queries when full_text mode is requested", async () => {
    ...
    const messageFtsSpy = vi
      .spyOn(
        conversationStore as unknown as { searchFullText: (...args: unknown[]) => unknown[] },
        "searchFullText",
      )
      .mockReturnValue(messageLikeResult);
    const messageLikeSpy = vi
      .spyOn(
        conversationStore as unknown as { searchLike: (...args: unknown[]) => unknown[] },
        "searchLike",
      )
      .mockReturnValue(messageLikeResult);
    const summaryFtsSpy = vi
      .spyOn(
        summaryStore as unknown as { searchFullText: (...args: unknown[]) => unknown[] },
        "searchFullText",
      )
      .mockReturnValue(summaryLikeResult);
    const summaryLikeSpy = vi
      .spyOn(
        summaryStore as unknown as { searchLikeCjk: (...args: unknown[]) => unknown[] },
        "searchLikeCjk",
      )
      .mockReturnValue(summaryLikeResult);
