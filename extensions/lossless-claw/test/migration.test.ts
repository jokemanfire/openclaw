...
describe("runLcmMigrations summary depth backfill", () => {
  it("adds depth and metadata from summary lineage", () => {
    ...
    expect(summaryColumns.some((column) => column.name === "source_message_token_count")).toBe(
      true,
    );
  ...
  it("replaces global session_key uniqueness with active-row uniqueness", () => {
    ...
    expect(queryPlanDetails.some((detail) => detail.includes("USE TEMP B-TREE FOR ORDER BY"))).toBe(
      false,
    );
  ...
  it("backfills message identity hashes and indexes conversation hash lookups", () => {
    ...
    const indexColumns = db
      ...
      .all() as Array<{ name?: string }>;
    expect(indexColumns.map((column) => column.name)).toEqual(["conversation_id", "identity_hash"]);
  });
  ...
  it("backfills message identity hashes across multiple batches", () => {
    ...
    const unhashedCount = db
      .prepare(
        `SELECT COUNT(*) AS count FROM messages WHERE identity_hash IS NULL OR identity_hash = ''`,
      )
      .get() as { count: number };
    ...
    expect(sampledRow.identity_hash).toBe(
      buildMessageIdentityHash("assistant", "batch message 1204"),
    );
  });
  ...
  it("recreates summaries_fts when the schema probe throws", () => {
    ...
    db.prepare(
      `INSERT INTO conversations (conversation_id, session_id, title) VALUES (?, ?, ?)`,
  ...
  it("creates message_parts when the bulk schema create did not", () => {
    ...
    const instrumentedDb = {
      ...
      exec(sql: string) {
        ...
        expect(schemaWithoutMessageParts).not.toContain("CREATE TABLE IF NOT EXISTS message_parts");
  ...
  it("retries a versioned backfill cleanly after the state write fails", () => {
    ...
    const failedConversationColumns = db
      .prepare(`PRAGMA table_info(conversations)`)
      .all() as Array<{
      name?: string;
    }>;
    const migrationStateTable = db
      .prepare(
        `SELECT name
       FROM sqlite_master
