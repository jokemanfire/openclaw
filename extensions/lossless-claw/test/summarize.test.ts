  ...
  describe("empty-summary retry and diagnostics", () => {
    it("does not enter conservative retry/fallback when the provider returns an auth error envelope", async () => {
      ...
      expect(diagnostics).toContain(
        "Check that the configured summaryProvider has valid API credentials.",
      );
    ...
    it("retries the same model with direct credentials before falling back to another provider", async () => {
      const deps = makeDeps({
        ...
        getApiKey: vi.fn(
          async (_provider: string, _model: string, options?: { skipModelAuth?: boolean }) =>
    ...
    it("retries when an incomplete message block still carries text output", async () => {
      ...
      const deps = makeDeps({
        ...
        complete: vi.fn(async () => {
              ...
              content: [
                {
                  ...
                  incomplete_details: { reason: "max_output_tokens" },
                  content: [
                    { type: "output_text", text: "Partial text hidden in incomplete item." },
                  ],
                },
              ],
            };
          ...
          return {
            content: [{ type: "text", text: "Recovered summary from incomplete item retry." }],
          };
        }),
      });
          ...
          output: [
            {
              type: "message",
              role: "assistant",
              content: [{ type: "output_text", text: "Summary from output message." }],
            },
          ],
        })),
      });
