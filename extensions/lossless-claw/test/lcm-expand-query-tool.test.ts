...
describe("createLcmExpandQueryTool", () => {
  ...
  it("returns a focused delegated answer for explicit summaryIds", async () => {
    ...
    let delegatedContext: ReturnType<typeof getDelegatedExpansionContextForTests> | undefined;
    ...
    callGatewayMock.mockImplementation(async (opts: unknown) => {
      ...
      if (request.method === "agent.wait") {
        ...
        if (waitCalls === 1) {
          return {
            status: "error",
            error: "provider/model overrides are not authorized for this caller.",
          };
        }
        return { status: "ok" };
      }
  ...
  it("falls back across session-family segments using per-conversation leaf links", async () => {
    ...
    summaryStore.getConversationMaxSummaryDepth.mockResolvedValueOnce(1).mockResolvedValueOnce(1);
