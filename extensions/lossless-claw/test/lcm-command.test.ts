...
function createCommandContext(args?: string, overrides: Record<string, unknown> = {}) {
  return {
    channel: "telegram",
...
describe("lcm command", () => {
  ...
  it("reports compact global status and help hints", async () => {
    ...
    expect(result.text).toContain(
      "OpenClaw did not expose an active session key or session id here",
    );
  });
  ...
  it("refuses session id fallback when it resolves to a different stored session key", async () => {
    ...
    expect(result.text).toContain("status: unavailable");
    expect(result.text).toContain(
      "Active session key `agent:main:telegram:direct:active` is not stored in LCM yet.",
    );
    expect(result.text).toContain(
      "but it is bound to `agent:main:telegram:direct:stored`, so Global stats are safer.",
    );
  ...
  it("reports doctor as unavailable when the current conversation cannot be resolved", async () => {
    ...
    expect(result.text).toContain(
      "fallback: Doctor is conversation-scoped, so no global scan ran.",
    );
  ...
  it("applies all doctor clean filters with backup-first deletion and preserves unrelated conversations", async () => {
    ...
    const quickCheck = fixture.db.prepare(`PRAGMA quick_check`).get() as
      | { quick_check?: string }
      | undefined;
    const remainingNormal = await fixture.conversationStore.getConversation(
      normalConversation.conversationId,
    );
    const removedArchived = await fixture.conversationStore.getConversation(
      archivedSubagent.conversationId,
    );
    const removedCron = await fixture.conversationStore.getConversation(
      cronConversation.conversationId,
    );
    const removedNull = await fixture.conversationStore.getConversation(
      nullSubagent.conversationId,
    );
    const remainingLiveNull = await fixture.conversationStore.getConversation(
      liveNullSubagent.conversationId,
    );
  ...
  it("applies a single doctor clean filter without deleting other candidate classes", async () => {
    ...
    const remainingArchived = await fixture.conversationStore.getConversation(
      archivedSubagent.conversationId,
    );
    const removedCron = await fixture.conversationStore.getConversation(
      cronConversation.conversationId,
  ...
  it("vacuums after doctor clean apply when requested", async () => {
    ...
    const walCheckpoint = fixture.db.prepare(`PRAGMA wal_checkpoint`).get() as
  ...
  it("warns when doctor clean apply quick_check reports integrity issues", async () => {
    ...
    const result = await command.handler(createCommandContext("doctor clean apply cron_sessions"));

    expect(result.text).toContain("status: warning");
    expect(result.text).toContain("quick_check: row 1 missing from index example_idx");
    expect(result.text).toContain(
      "writes committed, but SQLite integrity verification reported problems",
    );
  });
  ...
  it("repairs scoped doctor summaries in place and feeds repaired children into parents", async () => {
    const summarize = vi.fn(
      async (text: string, _aggressive?: boolean, options?: Parameters<LcmSummarizeFn>[2]) => {
  ...
  it("reports doctor apply as unavailable when the current conversation cannot be resolved and does not repair globally", async () => {
    ...
    expect(result.text).toContain(
      "fallback: Doctor apply is conversation-scoped, so no global repair ran.",
    );
    expect(result.text).not.toContain("detected summaries:");
    expect(summarize).not.toHaveBeenCalled();
  ...
  it("rotates the current session and replaces the latest rotate backup", async () => {
    ...
    expect(result.text).toContain(
      "mode: preserved current conversation and rotated transcript tail",
    );
    expect(backupPath).toBeTruthy();
  ...
  it("resolves the runtime session id from the session key when rotate lacks ctx.sessionId", async () => {
    const transcriptPath = join(
      tmpdir(),
      `lossless-claw-rotate-runtime-session-id-${Date.now()}.jsonl`,
    );
    writeFileSync(
      transcriptPath,
      '{"message":{"role":"user","content":[{"type":"text","text":"existing"}]}}\n',
  ...
  it("falls back to the stored conversation session id when runtime rotate resolution is unavailable", async () => {
    const transcriptPath = join(
      tmpdir(),
      `lossless-claw-rotate-stored-session-id-${Date.now()}.jsonl`,
    );
    writeFileSync(
      transcriptPath,
      '{"message":{"role":"user","content":[{"type":"text","text":"existing"}]}}\n',
