...
describe("LcmContextEngine ignored sessions", () => {
  ...
  it("skips afterTurn for ignored sessions while persisting included sessions", async () => {
    ...
    const stored = await engine
      .getConversationStore()
      .getMessages(includedConversation!.conversationId);
...
describe("LcmContextEngine.bootstrap", () => {
  ...
  it("preserves existing conversation data when the session file rotates", async () => {
    ...
    await engine
      .getSummaryStore()
      .appendContextSummary(conversation!.conversationId, "sum_rotation_old");
  ...
  it("preserves conversation history when the session file rotates across a stable sessionKey", async () => {
    ...
    expect(rotatedBootstrapState?.sessionFilePath).toBe(rotatedSessionFile);
    expect(
      await engine.getSummaryStore().getSummary("sum_rotation_session_key_old"),
    ).not.toBeNull();
  });
  ...
  it("rotates to a fresh conversation when a stable sessionKey resumes on a new transcript after the old file disappears", async () => {
    ...
    const archivedConversation = await engine
      .getConversationStore()
      .getConversation(originalConversation!.conversationId);
  ...
  it("preserves the active conversation when the tracked transcript stat fails for a non-missing reason", async () => {
    ...
    expect(
      await engine.getSummaryStore().getSummary("sum_stat_failure_fallback_old"),
    ).not.toBeNull();
  });
  ...
  it("rotates the current transcript in place without replacing the conversation", async () => {
    ...
    const originalStoredMessages = await engine
      .getConversationStore()
      .getMessages(original!.conversationId);
    ...
    expect(
      rotatedBranchMessages.map((message) => (message.content[0] as { text: string }).text),
  ...
  it("startup scan logs one compact summary for quiet skips", async () => {
    ...
    const autoRotateWarnLogs = log.warn.mock.calls
      .map(([message]) => String(message))
      .filter((message) => message.startsWith("[lcm] auto-rotate:"));
    expect(autoRotateInfoLogs.filter((message) => message.includes("action=summary"))).toHaveLength(
      1,
    );
  ...
  it("startup batch creates one pre-rotation backup for multiple rotations", async () => {
    ...
    try {
      const firstBackupState = backupDb
        .prepare(
          `SELECT last_seen_size AS lastSeenSize FROM conversation_bootstrap_state WHERE conversation_id = ?`,
        )
  ...
  it("reconciles missing tail messages when JSONL advanced past LCM", async () => {
    ...
    expect(bootstrapState?.lastProcessedEntryHash).not.toBe(
      firstBootstrapState!.lastProcessedEntryHash,
    );
  });
  ...
  it("keeps the append-only fast path after heartbeat pruning changes the DB frontier", async () => {
    ...
    const storedAfterPrune = await engine
      .getConversationStore()
      .getMessages(conversation!.conversationId);
    expect(storedAfterPrune.map((message) => message.content)).toEqual([
      "seed user",
      "seed assistant",
    ]);
    ...
    const storedAfterAppend = await engine
      .getConversationStore()
      .getMessages(conversation!.conversationId);
  ...
  it("does not advance the bootstrap checkpoint when reconcile aborts at the import cap", async () => {
    ...
    const storedAfterCap = await engine
      .getConversationStore()
      .getMessages(conversation!.conversationId);
    expect(storedAfterCap.map((message) => message.content)).toEqual([
      "seed user",
      "seed assistant",
    ]);
  ...
  it("externalizes oversized file blocks during first-time bootstrap and still reconciles later tail messages", async () => {
    await withTempHome(async () => {
      ...
      const parts = await engine
        .getConversationStore()
        .getMessageParts(initiallyStored[0].messageId);
      expect(parts).toHaveLength(1);
      expect(parts[0].textContent).toContain("[LCM File: file_");
...
describe("LcmContextEngine.assemble canonical path", () => {
  ...
  it("logs previous and current divergence message summaries when assembled prefixes change", async () => {
    ...
    (
      ...
      serializedMessages: [
        JSON.stringify({ role: "assistant", content: "older different message" }),
      ],
  ...
  it("rebuilds raw function_call blocks from stored columns when raw arguments are objects", async () => {
    ...
    const db = (
      engine.getConversationStore() as unknown as {
...
describe("LcmContextEngine fidelity and token budget", () => {
  ...
  it("preserves structured toolResult content via message_parts and assembler", async () => {
    ...
    expect((assembledMessage.content as Array<{ content?: unknown }>)[0]?.content).toEqual([
  ...
  it("afterTurn resolves tokenBudget from runtimeContext and forwards it as legacyParams", async () => {
    ...
    const executeLeafCompactionCoreSpy = vi
      .spyOn(privateEngine, "executeLeafCompactionCore")
      .mockResolvedValue({
        ok: true,
        compacted: false,
        reason: "below threshold",
    ...
    expect(
      (executeLeafCompactionCoreSpy.mock.calls[0]?.[0] as { tokenBudget?: unknown }).tokenBudget,
    ).toBe(2048);
    expect(
      (executeLeafCompactionCoreSpy.mock.calls[0]?.[0] as { legacyParams?: unknown }).legacyParams,
    ).toBe(runtimeContext);
    expect(compactSpy).not.toHaveBeenCalled();
  });
  ...
  it("afterTurn schedules a deferred drain even when compactionTelemetry has no provider/model (D4)", async () => {
    ...
    expect(
      infoMessages.some(
        (m) => m.includes("scheduled without cache context") && m.includes("cache-context-unknown"),
      ),
    ).toBe(true);
    expect(
  ...
  it("assemble() uses cold-cache catch-up passes when stale Anthropic debt overrides hot-cache smoothing", async () => {
    ...
    const privateEngine = engine as unknown as {
      evaluateIncrementalCompaction: (params: unknown) => Promise<unknown>;
      compaction: {
        compactLeaf: (input: { allowCondensedPasses?: boolean }) => Promise<unknown>;
      };
    };
  ...
  it("afterTurn persists prompt-cache telemetry for hot sessions", async () => {
    ...
    expect(debugLog).toHaveBeenCalledWith(expect.stringContaining("cacheState=hot"));
  });
  ...
  it("evaluateIncrementalCompaction skips hot-cache maintenance when real budget headroom is comfortable", async () => {
    ...
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("tokenBudget=100000"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("currentTokenCount=10000"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cacheRead=2048"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cacheWrite=null"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cachePromptTokenCount=10000"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cacheReadSharePct=20.5%"));
  });
  ...
  it("evaluateIncrementalCompaction treats low cache-read share as cold even when telemetry says hot", async () => {
    ...
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("reason=cold-cache-catchup"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cacheReadSharePct=15.0%"));
  });
  ...
  it("evaluateIncrementalCompaction keeps cache-write-only telemetry hot", async () => {
    ...
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cacheReadSharePct=0.0%"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("cacheWrite=8000"));
  });
  ...
  it("evaluateIncrementalCompaction scales budget-trigger passes by prompt overage", async () => {
    ...
    expect(decision.allowCondensedPasses).toBe(true);
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("reason=budget-trigger"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("maxPasses=4"));
  });
  ...
  it("afterTurn increases the working leaf chunk target for busy sessions when dynamic sizing is enabled", async () => {
    ...
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("activityBand=high"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("preferredLeafChunkTokens=40000"));
  });
...
describe("LcmContextEngine afterTurn dedup guard", () => {
  ...
  it("handles empty batch after slicing", async () => {
    ...
    await engine.afterTurn({
      ...
      messages: [makeMessage({ role: "system", content: "system prompt" })],
      prePromptMessageCount: 1,
      tokenBudget: 4096,
    });
  ...
  it("does not treat a one-message terminal match as a fully replayed batch", async () => {
    ...
    const conversation = await engine.getConversationStore().getConversationBySessionId(sessionId);
    const stored = await engine.getConversationStore().getMessages(conversation!.conversationId);
    expect(stored.map((m) => m.content)).toEqual(["same answer", "new question", "same answer"]);
  });
  ...
  it("preserves a legitimate repeated first new message", async () => {
    ...
    const conversation = await engine.getConversationStore().getConversationBySessionId(sessionId);
    const stored = await engine.getConversationStore().getMessages(conversation!.conversationId);
    expect(stored.map((m) => m.content)).toEqual(["hello", "first reply", "hello", "second reply"]);
  });
});
...
describe("LcmContextEngine compaction telemetry", () => {
  it("does not append synthetic system messages for compaction passes", async () => {
    ...
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("[lcm] LCM compaction leaf pass"));
  });
  ...
  it("compactLeafAsync logs cache-aware start details at info", async () => {
    ...
    expect(result.compacted).toBe(true);
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("[lcm] compactLeafAsync start:"));
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("leafChunkTokens=40000"));
    expect(infoLog).toHaveBeenCalledWith(
      expect.stringContaining("fallbackLeafChunkTokens=40000,30000,20000"),
    );
    expect(infoLog).toHaveBeenCalledWith(expect.stringContaining("activityBand=medium"));
  });
...
describe("LcmContextEngine.compact token budget plumbing", () => {
  ...
  it("supports openai-codex large-file summarization without direct-credential retry", async () => {
    const completeSpy = vi.fn(async ({ apiKey }: { apiKey?: string }) => ({
      content:
        apiKey === "scoped-token" ? [] : [{ type: "text", text: "codex large-file summary" }],
      ...(apiKey === "scoped-token"
        ? {
            error: {
              kind: "provider_auth",
    ...
    const privateEngine = engine as unknown as {
      resolveLargeFileTextSummarizer: () => Promise<
...
describe("LcmContextEngine.assemble maxAssemblyTokenBudget cap", () => {
  ...
  it("caps token budget in compactLeafAsync when maxAssemblyTokenBudget is set", async () => {
    ...
    const compactLeafSpy = vi.spyOn(privateEngine.compaction, "compactLeaf").mockResolvedValue({
      actionTaken: true,
      tokensBefore: 6000,
      tokensAfter: 3500,
      condensed: false,
