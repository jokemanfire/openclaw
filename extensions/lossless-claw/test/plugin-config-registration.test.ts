...
describe("lcm plugin registration", () => {
  ...
  it("uses api.pluginConfig values during register", { timeout: 20000 }, () => {
    ...
    expect(infoLog).toHaveBeenCalledWith("[lcm] Compaction summarization model: (unconfigured)");
  ...
  it.each([
    ...
    ["empty", {}],
  ])("falls back to root plugin config when api.pluginConfig is %s", (_label, pluginConfig) => {
    const dbPath = join(tmpdir(), `lossless-claw-${Date.now()}-${Math.random().toString(16)}.db`);
    dbPaths.add(dbPath);
  ...
  it("registers without runtime.modelAuth on older OpenClaw runtimes", () => {
    ...
    expect(warnLog).toHaveBeenCalledWith(
      expect.stringContaining("runtime.modelAuth is unavailable"),
    );
  });
  ...
  it(
    "falls back to auth-profiles.json when runtime.modelAuth is unavailable",
    { timeout: 20000 },
    async () => {
