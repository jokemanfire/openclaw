    ...
    typeof lastMsg.content === "string"
      ...
      : Array.isArray(lastMsg.content)
        ? lastMsg.content
            ...
            .join("")
        : "";
  const entryHash = createHash("sha256").update(JSON.stringify({ role, content })).digest("hex");
...
describe("bootstrap flood regression (PR #280) — round-trip integration", () => {
  ...
  it("bootstrap imports 0 messages after maintain rewrites JSONL and updates checkpoint", async () => {
    ...
    const dbCountAfterBoot = await engine.getConversationStore().getMessageCount(conversationId);
    ...
    const dbCountAfterReboot = await engine.getConversationStore().getMessageCount(conversationId);
    expect(dbCountAfterReboot, "message count should not increase after second bootstrap").toBe(
      dbCountAfterBoot,
    );
  });
  ...
  it("import cap fires when checkpoint is stale (corrupt mtime), blocking the flood", async () => {
    ...
    expect(boot2.reason, "should report import cap was hit").toBe("reconcile import capped");
    expect(boot2.importedMessages, "should import 0 messages when cap fires").toBe(0);

    // Verify no new messages were added to DB
    const dbCountAfterFlood = await engine.getConversationStore().getMessageCount(conversationId);
    expect(dbCountAfterFlood, "DB message count should not change after capped import").toBe(
      dbCountAfterBoot,
    );
  });
  ...
  it("no duplicate messages or seq numbers after bootstrap-maintain-bootstrap cycle", async () => {
    ...
    expect(messagesAfter.length, "message count should be identical after re-bootstrap").toBe(
    ...
    expect(uniqueSeqs.size, "all seq numbers should be unique — no duplicate rows").toBe(
      seqNumbers.length,
    );
  });
  ...
  it("both fixes work together: maintain updates checkpoint AND cap protects against stale state", async () => {
    ...
    expect(finalCount, "DB message count should be unchanged after capped flood attempt").toBe(
      dbCountAfterBoot,
    );
  });
});
